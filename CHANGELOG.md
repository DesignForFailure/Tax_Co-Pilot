...
### Added
- **Authoring for non-coders (Milestone 33).** Field feedback said tax accountants can't write YAML — this closes the gap from both directions:
  - **AI Authoring Assistant** (`/rule-packs/ai-assist`): generates a complete, schema-aware prompt from a plain-language description — rule-type contract, expression allowlist, manifest fields, required rule IDs, and the *live* reference catalog (engine `input.*` namespace derived from the calculator itself via the new `known_input_refs()`, the shipped pack's rule IDs and constants, federal cross-pack targets for states). The user pastes it into any AI they already use; the app never contacts an AI service (local-first, zero keys, zero egress).
  - **Paste-to-import**: the import page accepts one combined document using the export sentinels (`# === MANIFEST ===` / `# === RULES ===`), strips AI-chat prose and Markdown fences structurally, validates via the existing `RulePack.load()` gate with rollback, and on rejection preserves the paste and prompts the copy-the-error-back-to-your-AI round trip. Export → paste re-import round-trips.
  - **Constants editor**: pack constants (the amount tables every `lookup` rule reads) are now editable in the UI — flat filing-status tables and two-level grouped shapes, the only shapes shipped packs use. New `save_constant`/`delete_constant` service functions use the same validate-before-write discipline as `save_rule`; deleting a constant a lookup rule still references is refused (the loader wouldn't catch the dangling path until calculation time). Previously constants could not be edited anywhere — `lookup`-based logic was impossible to author in the GUI.
  - **`matrix_lookup` grid editor**: the fifth rule type joins the web editor as a row×column grid (covers every matrix rule shipped; deeper matrices still say "edit via YAML"), replacing the hard form-parser rejection.
  - **Reference autocomplete + guidance**: every reference field offers datalist suggestions (pack rule IDs, `constants.*` paths, `input.*` catalog, federal IDs for state packs); bracket tables gained copy-rows-from-single, a rate-format hint, and an "Upper blank = no ceiling" explanation; plain-language ref-vs-literal help; docs links from the editor.
  - Coverage the survey flagged as missing now exists: route-level round-trips (with on-disk YAML read-back) for lookup, bracket_table, matrix_lookup, and literal formula inputs; an end-to-end test authoring a pack purely from form-encoded data and calculating a return with it; `test_save_rule_via_post` no longer accepts the error page as a pass. 747 tests total (was 698).

### Security
- **Path-traversal containment guard (CodeQL follow-up).** `rule_pack_editor._pack_path` now confirms the composed pack directory stays under the pack root via `os.path.normpath` + a prefix check — the filesystem-free containment sanitizer CodeQL's `py/path-injection` guidance recommends. This is genuine defense in depth behind the existing segment validators (which already reject `..`/`/`/`\` and constrain jurisdiction/variant/year) and clears the 42 remaining `py/path-injection` alerts without dismissals. Behavior is unchanged for valid inputs.

### Security
- **CodeQL code-scanning triage.** Enabled GitHub code scanning surfaced 60 alerts (50 ranked High); all 60 were triaged against the code. **Real hardening applied:** a length guard in `collect_indices` before the `IDX_RE` match to remove a polynomial-ReDoS vector on attacker-controlled form-field *names*; generic client error messages (with the detail logged server-side) for the JSON-import, database-restore, and pack-export failure paths (`py/stack-trace-exposure`); and a least-privilege `permissions: contents: read` block on the CI workflow. **Dismissed as verified false positives** (with written justifications on each alert): 42 `py/path-injection` alerts (the rule-pack editor builds every path from segments already validated by `_validate_path_param` — which rejects `..`, `/`, and `\` — plus the `^[a-zA-Z]{2,10}$` jurisdiction and `^(standard|custom_v\d+)$` variant regexes and a bounded year, so traversal is impossible; a `resolve()`-based containment guard was trialed but only relocated the flag to the guard's own `resolve()` call, which CodeQL treats as a sink); five `py/clear-text-logging` alerts (they log the `password_source` config enum, never the password), two `js/xss-through-dom` alerts (innerHTML built from hardcoded `data-*` tokens and integer counters), three `py/url-redirection` alerts (fixed internal `/rule-packs/` path with regex-validated segments), and three `py/stack-trace-exposure` alerts on self-authored, fixed guidance messages. Covered by a strengthened path-validation test in `tests/test_rule_pack_editor.py`.
...
### Added
- **Roadmap Milestone 32 (complete): Pennsylvania, Illinois, and North Carolina packs (2024, v1.0.0 each).** **PA**: flat 3.07% on class income built from PA's own income classes — compensation, interest, dividends, net gains, net profits, each floored at zero (a loss in one class never offsets another; no deductions/exemptions; Social Security untaxed by construction; Tax Forgiveness and reciprocity documented unmodeled). **IL**: flat 4.95% on federal AGI minus federally taxed Social Security, less the $2,775-per-person exemption allowance (verified against FY Bulletin 2024-02) with $1,000 senior/blind additions (using the M25 flags) and full disallowance above $250k/$500k AGI (boundary pinned); retirement subtraction and IL credits documented unmodeled. **NC**: flat 4.5% (the statutory 2023→2025 glide path) on federal AGI minus taxed Social Security and the $12,750/$25,500/$19,125 standard deduction; the child deduction documented unmodeled. All three ship with the M31 apportionment/other-state-credit tail, verified by a GA-resident-with-PA-wages vector. Six states with income tax now covered (plus the seven no-tax stubs). Covered by `tests/test_state_pa_il_nc.py` (10 tests).

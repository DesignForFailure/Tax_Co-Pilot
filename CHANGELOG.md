...
### Security
- **CodeQL code-scanning triage.** Enabled GitHub code scanning surfaced 60 alerts (50 ranked High); all 60 were triaged against the code. **Real hardening applied:** a defense-in-depth path-containment check in `rule_pack_editor._pack_path` (the resolved pack directory must stay inside the pack root — this is belt-and-suspenders behind the existing segment validators and clears the 42 `py/path-injection` alerts, including the downstream `rule_loader`/`calculate` sites); a length guard in `collect_indices` before the `IDX_RE` match to remove a polynomial-ReDoS vector on attacker-controlled form-field *names*; generic client error messages (with the detail logged server-side) for the JSON-import, database-restore, and pack-export failure paths (`py/stack-trace-exposure`); and a least-privilege `permissions: contents: read` block on the CI workflow. **Dismissed as verified false positives** (with written justifications on each alert): five `py/clear-text-logging` alerts (they log the `password_source` config enum, never the password), two `js/xss-through-dom` alerts (innerHTML built from hardcoded `data-*` tokens and integer counters), three `py/url-redirection` alerts (fixed internal `/rule-packs/` path with regex-validated segments), and three `py/stack-trace-exposure` alerts on self-authored, fixed guidance messages. Covered by two new containment tests in `tests/test_rule_pack_editor.py`.

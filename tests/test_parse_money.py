...
"""Unit tests for parse_money monetary input boundary."""

from decimal import Decimal

import pytest

from app.route_helpers.form_parsing import parse_money


class TestValidInputs:
    def test_integer(self) -> None:
        assert parse_money("75000") == Decimal("75000.00")

    def test_with_decimals(self) -> None:
        assert parse_money("75000.50") == Decimal("75000.50")
    ...
    def test_just_below_billion(self) -> None:
        assert parse_money("999999999") == Decimal("999999999.00")
...
class TestRejectedInputs:
    ...
    def test_negative_allowed_when_opted_in(self) -> None:
        assert parse_money("-500", allow_negative=True) == Decimal("-500.00")
